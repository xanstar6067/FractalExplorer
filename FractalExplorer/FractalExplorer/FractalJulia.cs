...
namespace FractalDraving
{
    ...
    public partial class FractalJulia : Form, IFractalForm
    {
        ...
        // ИЗМЕНЕНИЕ: Комплексное число 'c', определяющее конкретный фрактал Жюлиа
        ...
        // ИЗМЕНЕНИЕ: Константы для отображения множества Мандельброта (используется для выбора 'c')
        ...
        private async void Form1_Load(object sender, EventArgs e)
        {
            ...
            // ИЗМЕНЕНИЕ: Увеличение точности для Re и Im
            nudRe1.Minimum = -2m;
            nudRe1.Maximum = 2m;
            nudRe1.DecimalPlaces = 15; // Увеличена точность
            nudRe1.Increment = 0.001m;

            nudIm1.Minimum = -2m;
            nudIm1.Maximum = 2m;
            nudIm1.DecimalPlaces = 15; // Увеличена точность
        ...
        /// <summary>
        /// Отрисовка с учетом трансформации для "мгновенного" панорамирования/зума.
        /// </summary>
        ...
        /// <summary>
        /// Внутренний метод для рендеринга множества Мандельброта.
        /// </summary>
        ...
        {
            // ИЗМЕНЕНИЕ: Конвертируем в decimal для установки в NUD
        ...
        private async void RenderTimer_Tick(object sender, EventArgs e)
        {
            ...
            try
            {
                // ИЗМЕНЕНИЕ: Передаем decimal в метод рендеринга
        ...
        {
            // ИЗМЕНЕНИЕ: Используем ComplexDecimal и вычисляем thresholdSquared
        ...
        /// </summary>
        // ИЗМЕНЕНИЕ: Сигнатура метода для приема decimal и ComplexDecimal
        private Bitmap RenderFractalToBitmap(int renderWidth, int renderHeight, decimal currentCenterX, decimal currentCenterY,
                                             decimal currentZoom, decimal currentBaseScale, ComplexDecimal currentC_param,
        ...
        private void Canvas_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            centerX -= (decimal)(e.X - panStart.X) * scale / width;
            centerY -= (decimal)(e.Y - panStart.Y) * scale / height;
            panStart = e.Location;
        ...
        private async void btnSave_Click_1(object sender, EventArgs e)
        {
            ...
            // ИЗМЕНЕНИЕ: Форматирование имени файла для высокой точности
            ...
            using (SaveFileDialog saveDialog = new SaveFileDialog { Filter = "PNG Image|*.png", Title = "Сохранить фрактал Жюлиа (Высокое разрешение)", FileName = suggestedFileName })
            {
                if (saveDialog.ShowDialog() == DialogResult.OK)
                {
                    ...
                    try
                    {
                        ...
                        Bitmap highResBitmap = await Task.Run(() => RenderFractalToBitmap(
                            ...
                            BASE_SCALE, currentC_Capture,
                            currentMaxIterations_Capture, currentThreshold_Capture, currentThreadCount_Capture,
                            progressPercentage =>
                            {
