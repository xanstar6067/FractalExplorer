...
namespace FractalExplorer.Forms
{
    public partial class FractalNovaJuliaForm : Form, IFractalForm, IHighResRenderable, ISaveLoadCapableFractal
    {
        ...
        private void ParamControl_Changed(object sender, EventArgs e)
        {
            ...
            // If C changed, we update the marker on the map
        ...
        private void ParamControl_Changed_WithMapUpdate(object sender, EventArgs e)
        {
            ...
            // When P, M or Z0 changes, the shape of the Mandelbrot set (the map) changes.
            // We need to re-render the map AND the Julia set.
        ...
        private async void RenderMandelbrotMapAsync()
        {
            ...
            try
            {
                Bitmap mapBmp = await Task.Run(() =>
                    mapEngine.RenderToBitmap(w, h, 1, _ => { }, CancellationToken.None));
        ...
        private void PbMandelbrotPreview_Paint(object sender, PaintEventArgs e)
        {
            ...
            // Map assumes Center (0,0) and range ~4.0 (-2 to 2)
        ...
        private void PbMandelbrotPreview_Click(object sender, EventArgs e)
        {
            ...
            {
                // Pass current P, M, Z0 to the selector so it renders the correct map
                _selectorForm = new NovaMandelbrotSelectorForm(
                    this,
                ...
                _selectorForm.CoordinatesSelected += (re, im) =>
                {
                    nudC_Re.Value = (decimal)re;
                    nudC_Im.Value = (decimal)im;
                };
        ...
        private void UpdateEngineParameters()
        {
            ...
            _fractalEngine.JuliaC = new Resources.ComplexDecimal(nudC_Re.Value, nudC_Im.Value);
        ...
        private FractalNovaFamilyEngine CreateEngineFromState(HighResRenderState state, bool forPreview)
        {
            ...
            engine.JuliaC = state.JuliaC ?? new ComplexDecimal(0, 1);
        ...
        public async Task<byte[]> RenderPreviewTileAsync(FractalSaveStateBase state, TileInfo tile, int totalWidth, int totalHeight, int tileSize)
        {
            return await Task.Run(() =>
            {
                ...
                NovaJuliaPreviewParams previewParams;
                try { previewParams = JsonSerializer.Deserialize<NovaJuliaPreviewParams>(state.PreviewParametersJson); }
                ...
                var previewEngine = new NovaJuliaEngine
                {
                    ...
                    M = previewParams.M,
                    JuliaC = new ComplexDecimal(previewParams.C_Re, previewParams.C_Im),
        ...
        public Bitmap RenderPreview(FractalSaveStateBase state, int previewWidth, int previewHeight)
        {
            if (string.IsNullOrEmpty(state.PreviewParametersJson))
            {
                ...
                using (var g = Graphics.FromImage(bmpError)) { g.Clear(Color.DarkGray); }
                return bmpError;
            }
            NovaJuliaPreviewParams previewParams;
            try { previewParams = JsonSerializer.Deserialize<NovaJuliaPreviewParams>(state.PreviewParametersJson); }
            ...
            {
                var bmpError = new Bitmap(previewWidth, previewHeight);
                using (var g = Graphics.FromImage(bmpError)) { g.Clear(Color.DarkRed); }
                return bmpError;
            }
        ...
        {
            var specificSaves = SaveFileManager.LoadSaves<NovaJuliaSaveState>(this.FractalTypeIdentifier);
            return specificSaves.Cast<FractalSaveStateBase>().ToList();
        }

        public void SaveAllSavesForThisType(List<FractalSaveStateBase> saves)
        {
            var specificSaves = saves.Cast<NovaJuliaSaveState>().ToList();
            SaveFileManager.SaveSaves(this.FractalTypeIdentifier, specificSaves);
        }

        #region IFractalForm Implementation

        // Этот интерфейс требует селектор. 
        // Мы возвращаем стандартное значение, так как в этой форме пока нет настройки "Лупы".
        public double LoupeZoom => 4.0;

        // Событие требуется интерфейсом, но мы его пока не используем.
