...
namespace FractalExplorer.Forms
{
    ...
    public partial class FractalPhoenixForm : Form, ISaveLoadCapableFractal, IHighResRenderable
    {
        ...
        /// </summary>
        private PhoenixEngine _fractalEngine = null!;
        /// <summary>
        ...
        private void Canvas_Paint(object? sender, PaintEventArgs e)
        {
            ...
            lock (_bitmapLock)
            {
                ...
                if (_currentRenderingBitmap != null && (!_isUserResizingWindow || !drewStablePreview)) DrawVisibleBitmap(e.Graphics, _currentRenderingBitmap, currentViewportRect, canvas.ClientRectangle);
            }
        ...
        private Func<double, Color> GenerateSmoothPaletteFunction(Palette palette, int effectiveMaxColorIterations)
        {
            ...
            // Общий случай для всех остальных палитр.
        ...
        public async Task<byte[]> RenderPreviewTileAsync(FractalSaveStateBase state, TileInfo tile, int totalWidth, int totalHeight, int tileSize)
        {
            return await Task.Run(() =>
            {
                ...
                try { previewParams = JsonSerializer.Deserialize<PhoenixPreviewParams>(state.PreviewParametersJson) ?? throw new JsonException("Invalid preview parameters."); }
        ...
        private PhoenixEngine CreateEngineFromState(HighResRenderState state, bool forPreview)
        {
            ...
            engine.C1 = state.JuliaC ?? throw new InvalidOperationException("Для Phoenix не задан параметр C1.");
