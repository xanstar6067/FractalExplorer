using FractalExplorer.Core; // ДОБАВЛЕНО
...
namespace FractalDraving
{
    public abstract partial class FractalFormBase : Form, IFractalForm
    {
        ...
        // --- УДАЛЕНО: Старое управление палитрами ---
        ...
        {
            // --- ДОБАВЛЕНО: Инициализация менеджера палитр ---
            ...
            // --- ИЗМЕНЕНО: Скрываем старые чекбоксы и инициализируем новые контролы ---
            ...
            // --- ИЗМЕНЕНО: Применяем палитру по умолчанию ---
            ApplyActivePalette();
            ScheduleRender();
        }

        // --- ДОБАВЛЕНО: Метод для скрытия старых контролов ---
        ...
        // ... (Методы InitializeControls, StartPreviewRender, GenerateTiles и другие остаются прежними)
        #region Unchanged_Methods_For_Brevity
        ...
        private async Task StartPreviewRender()
        {
            ...
            catch (OperationCanceledException)
            {
                lock (_bitmapLock)
                {
                    if (_currentRenderingBitmap == newRenderingBitmap) { _currentRenderingBitmap?.Dispose(); _currentRenderingBitmap = null; }
                }
        ...
        private void InitializeEventHandlers()
        {
            ...
            // Убедитесь, что у вашей кнопки в дизайнере имя color_configurations
            ...
            // --- УДАЛЕНО: Подписки для старых чекбоксов ---
        ...
        // --- ДОБАВЛЕНО: Обработчик клика по кнопке конфигурации цвета ---
        private void color_configurations_Click(object sender, EventArgs e)
        {
            using (var form = new ColorConfigurationForm(_paletteManager))
            {
                ...
                {
                    // Пользователь нажал "Применить", обновляем палитру и перерисовываем
                    ApplyActivePalette();
                    ScheduleRender();
                }
            }
        ...
        // --- ДОБАВЛЕНО: Применение выбранной палитры к движку ---
        ...
        // --- УДАЛЕНЫ СТАРЫЕ МЕТОДЫ УПРАВЛЕНИЯ ПАЛИТРОЙ ---
        // PaletteCheckBox_CheckedChanged, HandlePaletteSelectionLogic, GetPaletteFuncByName и все GetPalette...Color

        #region Unchanged_Event_Handlers
        ...
        private void ParamControl_Changed(object sender, EventArgs e)
        {
            ...
            if (sender == nudZoom) { _zoom = nudZoom.Value; }
            ScheduleRender();
        }
        ...
        // --- ДОБАВЛЕНО: Главный метод генерации функции цвета из палитры ---
        private Func<int, int, int, Color> GeneratePaletteFunction(ColorPalette palette)
        {
            // Захватываем копии, чтобы избежать проблем с многопоточностью, если палитра изменится
            var colors = new List<Color>(palette.Colors);
            bool isGradient = palette.IsGradient;
            int colorCount = colors.Count;

            // Если цветов нет или один, возвращаем простую функцию
        ...
        // --- ДОБАВЛЕНО: Вспомогательный метод для интерполяции ---
        ...
        private void UpdateEngineParameters()
        {
            ...
            // --- ДОБАВЛЕНО: Применение палитры при каждом обновлении параметров ---
            ApplyActivePalette();
        }
            ...
            })
            {
                if (saveDialog.ShowDialog() == DialogResult.OK)
                {
                    ...
                    try
                    {
                        ...
                        // Применяем текущую активную палитру
