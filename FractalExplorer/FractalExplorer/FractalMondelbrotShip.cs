...
namespace FractalDraving
{
    ...
    public partial class FractalMondelbrotShip : Form
    {
        ...
        // Начальные константы для Горящего Корабля
        ...
        {
            InitializeComponent();
            this.centerX = (INITIAL_MIN_RE_BS + INITIAL_MAX_RE_BS) / 2.0;
            this.centerY = (INITIAL_MIN_IM_BS + INITIAL_MAX_IM_BS) / 2.0;

            double initialRangeX = INITIAL_MAX_RE_BS - INITIAL_MIN_RE_BS;
            double initialRangeY = INITIAL_MAX_IM_BS - INITIAL_MIN_IM_BS;
            double initialViewRange = Math.Max(initialRangeX, initialRangeY);
        ...
        // --- Все методы до UpdateParameters, включая палитры, остаются практически без изменений. ---
        // --- Я скопирую их для полноты, но ключевые изменения - в UpdateParameters и Render-методах. ---
        #region Unchanged_UI_Handlers_And_Palettes
            ...
            try
            {
                ...
                Parallel.For(0, height, po, y =>
                {
                    ...
                    for (int x = 0; x < width; x++)
                    {
                        ...
                        // ИЗМЕНЕНИЕ: Формула Горящего Корабля и проверка по квадрату модуля
