...
namespace FractalDraving
{
    ...
    /// </summary>
    public partial class FractalburningShipComplex : Form, IFractalForm
    {
        ...
        // Константы для отображения "карты" фрактала (используется для выбора 'c')
        // Для Горящего Корабля лучше использовать другие границы, чтобы он был виден полностью
        private const double MANDELBROT_MIN_RE = -2.0;
        private const double MANDELBROT_MAX_RE = 1.5;
        private const double MANDELBROT_MIN_IM = -1.5;
        private const double MANDELBROT_MAX_IM = 1.0;
        ...
        /// </summary>
        public FractalburningShipComplex()
        {
            InitializeComponent(); // Инициализация компонентов формы, созданных дизайнером
            this.Text = "Фрактал Горящий Корабль (Жюлиа)"; // Изменяем заголовок окна
        }
        ...
        private void MandelbrotCSelectorWindow_CoordinatesSelected(double re, double im)
        {
            ...
            bool changed = false;
            if (nudRe1.Value != (decimal)re) { nudRe1.Value = (decimal)re; changed = true; }
            if (nudIm1.Value != (decimal)im) { nudIm1.Value = (decimal)im; changed = true; }
        ...
        private void Canvas_MouseMove(object sender, MouseEventArgs e) { if (isHighResRendering || !panning) return; double scale = BASE_SCALE / zoom; centerX -= (e.X - panStart.X) * scale / width; centerY -= (e.Y - panStart.Y) * scale / height; panStart = e.Location; canvas1.Invalidate(); ScheduleRender(); }
